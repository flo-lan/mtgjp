...
const COLOR_MANA: Record<string, any> = {
  W: { Bg: ManaWSym, Vector: VectorWSym, inset: ['2.07%', '2.35%', '2.05%', '2.27%'] },
  U: { Bg: ManaUSym, Vector: VectorUSym, inset: ['10.54%', '24.55%', '8.97%', '25.34%'] },
  B: { Bg: ManaGeneric, Vector: VectorBSym, inset: ['7.77%', '9.33%', '7.75%', '9.25%'] },
  R: { Bg: ManaRSym, Vector: VectorRSym, inset: ['6.78%', '16.23%', '7.38%', '6.65%'] },
  G: { Bg: ManaGSym, Vector: VectorGSym, inset: ['6.4%', '6.45%', '4.78%', '6.22%'] },
  C: { Bg: ManaCSym, Vector: VectorCSym, inset: ['10%', '10%', '10%', '10%'] },
};

const NUMBER_VECTORS: Record<string, any> = {
  '0': { Vector: Vector0, inset: ['10.83%', '23.5%', '11.17%', '23%'] },
  '1': { Vector: Vector1, inset: ['8.67%', '32.17%', '12.83%', '31.5%'] },
  '2': { Vector: Vector2, inset: ['10.78%', '25.83%', '13%', '20.5%'] },
  '3': { Vector: Vector3, inset: ['10.45%', '26.17%', '11.33%', '21%'] },
  '4': { Vector: Vector4, inset: ['10.5%', '18.67%', '12.83%', '25.5%'] },
  '5': { Vector: Vector5, inset: ['10.67%', '25.17%', '11.33%', '24.83%'] },
  '6': { Vector: Vector6, inset: ['10.83%', '24.67%', '11.17%', '24.17%'] },
  '7': { Vector: Vector7, inset: ['10.5%', '24.17%', '12.67%', '24.17%'] },
  '8': { Vector: Vector8, inset: ['10.5%', '24.5%', '11.17%', '24.33%'] },
  '9': { Vector: Vector9, inset: ['10.83%', '24.4%', '11.17%', '24%'] },
};
...
const HYBRID_MANA: Record<string, any> = {
  'W/U': { Bg: HybridWuBg, Group: HybridGroupWu, inset: ['10.57%', '14.65%', '0%', '0%'] },
  'W/B': { Bg: HybridWuBg, Group: HybridGroupWb, inset: ['10.55%', '14.65%', '0%', '0%'] },
  'U/B': { Bg: HybridUbBg, Group: HybridGroupUb, inset: ['7%', '14.64%', '0%', '0.01%'] },
  'U/R': { Bg: HybridUrBg, Group: HybridGroupUr, inset: ['6.99%', '14.66%', '-0.01%', '-0.01%'] },
  'B/R': { Bg: HybridBrBg, Group: HybridGroupBr, inset: ['8.07%', '13.4%', '0%', '0%'] },
  'B/G': { Bg: HybridBrBg, Group: HybridGroupBg, inset: ['8.07%', '13.41%', '0%', '0%'] },
  'R/W': { Bg: HybridRwBg, Group: HybridGroupRw, inset: ['10.52%', '14.65%', '-0.01%', '0%'] },
  'R/G': { Bg: HybridRwBg, Group: HybridGroupRg, inset: ['10.52%', '14.66%', '-0.01%', '-0.01%'] },
  'G/W': { Bg: HybridGwBg, Group: HybridGroupGw, inset: ['11.64%', '12.02%', '-0.01%', '0%'] },
  'G/U': { Bg: HybridGwBg, Group: HybridGroupGu, inset: ['11.6%', '12.02%', '0%', '0%'] },
};
...
const PHYREXIAN_MANA: Record<string, any> = {
  ...
  'G/P': { Bg: PhyGp, Vector: PhyVectorGp, inset: ['0.98%', '22.25%', '1%', '22.27%'] },
};
...
export default function ManaSymbol({ symbol, size = 16, margin = 0 }: ManaSymbolProps) {
  ...
  const renderIcon = (BgComp: any, VectorComp: any, inset: string[]) => (
    <View style={[styles.manaSymbol, sizeStyle]}>
      ...
      {VectorComp && (
        <View style={[styles.manaSymbolVector, { top: inset[0], right: inset[1], bottom: inset[2], left: inset[3] }]}>
  ...
  if (s === 'T') {
    return renderIcon(ManaGeneric, VectorTSym, ['25.43%', '22.35%', '8.9%', '14.63%']);
  }
  ...
  return (
    <View style={[styles.manaSymbol, sizeStyle]}>
      ...
      <View style={styles.fallbackContainer}>
        <Text style={[styles.manaSymbolFallback, { fontSize: size * 0.5 }]}>{s}</Text>
      </View>
    </View>
  );
}
